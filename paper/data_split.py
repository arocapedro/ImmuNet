...
def split_bladder(annotations_path, output_dir=split_path):
    ...
    added_tiles = [
        "/".join((tile["ds"], tile["slide"], tile["tile"])) for tile in val_tiles
    ]
    ...
    tiles = tiles_info(dataset, tile_ids_exclude=added_tiles)
    filtered_tiles = [tile for tile in tiles if 5 <= tile["B cell"] < 50]
    random.shuffle(filtered_tiles)
...
def split_lung(annotations_path, output_dir=split_path):
    ...
    filtered_tiles = [
        tile
        for tile in tiles
        if (tile["No cell"] >= 5 or tile["Other cell"] >= 2) and tile["Other cell"] < 10
    ]
    # Include as few tumor cells as possible
    filtered_tiles = sorted(
        filtered_tiles, key=lambda x: x["Tumor cell"], reverse=False
    )
...
def split_melanoma(annotations_path, output_dir=split_path):
    ...
    filtered_tiles = [
        tile
        for tile in tiles
        if (tile["B cell"] >= 5 or tile["No cell"] >= 2) and tile["No cell"] < 11
    ]
...
def split_tonsils(annotations_path, output_dir=split_path):
    ...
    filtered_tiles = [
        tile
        for tile in tiles
        if tile["No cell"] >= 5 or tile["Other cell"] >= 5 or tile["Tumor cell"] >= 5
    ]
    # Include as few T and B cells as possible
    filtered_tiles = sorted(
        filtered_tiles, key=lambda x: x["T cell"] + x["B cell"], reverse=False
    )
...
def merge_split_by_tissue(split_dir=split_path):
    ...
    val_tiles_to_exclude = [
        "2020-01-31-phenotyping-paper-melanoma/T14-18776-I1_Scan1/56327,7224",
    ...
    for tissue in tissues:
        ...
        with open(split_dir / val_tiles_file) as f:
            ...
            tissue_val_tile_ids = [
                tile_id
                for tile_id in tissue_val_tile_ids
                if tile_id not in val_tiles_to_exclude
            ]
    ...
    for dataset in datasets:
        ...
        for slide in dataset["slides"]:
            ...
            tiles_filtered = [
                tile
                for tile in tiles
                if "/".join((dataset["ds"], slide["slide"], tile["tile"])) in tile_ids
            ]
    ...
    for dataset in datasets:
        ...
        for tile in dataset.tiles:
            for ann in tile.annotations:
                if detailed:
                    ann_cell_type = panel.annotation_phenotype(
                        ann["type"], ann["positivity"]
                    )
...
def save_bar_plots(stat_train, stat_val, split_stat_path):
    ...
    for tissue in tissues:
        ...
        cell_types = sorted(
            set(tissue_train_stat.keys()).union(set(tissue_val_stat.keys()))
        )
        ...
        for cell_type in cell_types:
            num_train = (
                tissue_train_stat[cell_type] if cell_type in tissue_train_stat else 0
            )
            nums_train.append(num_train)

            num_val = (
                tissue_val_stat[cell_type] if cell_type in tissue_train_stat else 0
            )
        ...
        bar_plot(
            ...
            nums_val,
            nums_train,
            "Annotation split {}".format(tissue),
            split_stat_path / "{}.png".format(tissue),
        )
...
def save_ann_stat(train_ann_path, val_ann_path, output_dir=split_stat_path):
    ...
    make_ann_stat_csv(
        stat_train_subtypes,
        stat_val_subtypes,
        output_dir / "train_val_split_subtypes.csv",
    )
...
if __name__ == "__main__":
    ...
    if stage == Stage.bladder:
        ...
        extract_annotations_on_tiles(
            annotations_file, val_tile_ids, val_annotations_path
        )
        ...
        extract_annotations_on_tiles(
            annotations_file, train_tile_ids, train_annotations_path, CYTOAGARS_DS
        )
